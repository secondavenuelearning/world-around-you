...
(function() {
  ...
  EPub = class EPub {
    constructor(options, output) {
      ...
      if (_.isString(this.options.signer)) {
        this.options.signer = [this.options.signer];
      }
      if (_.isEmpty(this.options.signer)) {
        this.options.signer = ["anonymous"];
      }
      if (_.isString(this.options.translator)) {
        this.options.translator = [this.options.translator];
      }
      if (_.isEmpty(this.options.translator)) {
        this.options.translator = ["none"];
      }
      ...
      this.options.content = _.map(this.options.content, function(content, index) {
        ...
        $($("*").get().reverse()).each(function(elemIndex, elem) {
          ...
          attrs = elem.attribs;
          that = this;
          if ((ref = that.name) === "img" || ref === "br" || ref === "hr" || ref === "source" || ref === "video") {
            if (that.name === "img") {
              $(that).attr("alt", $(that).attr("alt") || "image-placeholder");
            }
            if (that.name === "source") {
              $(that).attr("type", $(that).attr("type") || "video/mp4");
            }
            if (that.name === "video") {
              $(that).attr("poster", "cover.png");
            }
          }
          for (k in attrs) {
            v = attrs[k];
        ...
        $("source").each(function(index, elem) {
          var dir, extension, id, video, mediaType, url;
          var timestamp ="";
          url = $(elem).attr("src");
          if (video = self.options.videos.find(function(element) {
            return element.url === url;
          })) {
            id = video.id;
            ...
            extension = mime.getExtension(mediaType);
            dir = content.dir;
            self.options.videos.push({id, url, dir, mediaType, extension});
          }
          if(url.indexOf('#') != -1)
            timestamp = url.substring(url.indexOf('#'));
          $(elem).attr("type", $(elem).attr("type") || "video/mp4");
          return $(elem).attr("src", `images/${id}.${extension}` + timestamp);
        });
        content.data = $.xml();
        return content;
      });
    ...
    render() {
      ...
      return this.generateTempFile().then(function() {
        ...
        return self.downloadAllImage().fin(function() {
          ...
          return self.downloadAllVideos().then(function() {
          if (self.options.verbose) {
...
//Custom Features
    downloadVideo(options) { //{id, url, mediaType}
 
      var auxpath, downloadVideoDefer, filename, requestAction, self, userAgent;
      self = this;
      userAgent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_9_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.116 Safari/537.36";
      ...
      downloadVideoDefer = new Q.defer();
      ...
      if (options.url.indexOf("file://") === 0) {
        auxpath = options.url.substr(7);
        fsextra.copySync(auxpath, filename);
        return downloadVideoDefer.resolve(options);
      } else {
        if (options.url.indexOf("http") === 0) {
          requestAction = request.get(options.url).set({
            'User-Agent': userAgent
        ...
        requestAction.on('error', function(err) {
          console.error('[Download Error]', 'Error while downloading', options.url, err);
          fs.unlinkSync(filename);
          return downloadVideoDefer.reject(err);
        });
        requestAction.on('end', function() {
          console.log("[Download Success]", options.url);
          return downloadVideoDefer.resolve(options);
        });
        return downloadVideoDefer.promise;
      }
    }

    downloadAllVideos() {
      var deferArray, videoDefer, self;
      self = this;

      videoDefer = new Q.defer();
      if (!self.options.videos.length) {
        ...
        deferArray = [];
        _.each(self.options.videos, function(video) {
        	//console.log(self.options.videos);
          return deferArray.push(self.downloadVideo(video));
        });
        Q.all(deferArray).fin(function() {
          return videoDefer.resolve();
        });
      }
